...
export async function handlerSubMenuLocation(ctx, cbqData) {
	...
	try {
		...
		if (cbqData.includes('addLocationNew_***')) {
			await buttonEmpty.locationStart(ctx, cbqData);
			return;
		}
		...
		if (cbqData.includes('addLocationNew_')) {
			...
			const location = new Location({ botId, name: locationName, weather: [locationName] });
			await location.save();
		...
		if (cbqData.includes('removeLocationNew_')) {
			...
			await Location.findOneAndDelete({ botId, name: locationName });
